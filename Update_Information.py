...
def buscar_orders(conn):
    cursor = conn.cursor()
    data_limite = (datetime.now() - timedelta(days=10)).strftime('%Y-%m-%d %H:%M:%S')
    query = """
...
def atualizar_dados_order(conn, order_id, marketplace_fee, list_cost, seller_id, date_created, invoice_key, invoice_series, invoice_number, cost, sale_fee, updated):
    ...
    query = """
    UPDATE meli_orders
    SET marketplace_fee = ISNULL(?, marketplace_fee), 
        list_cost = ISNULL(?, list_cost), 
        seller_id = ISNULL(?, seller_id), 
        date_created = ISNULL(?, date_created), 
        invoice_key = ISNULL(?, invoice_key), 
        invoice_series = ISNULL(?, invoice_series), 
        invoice_number = ISNULL(?, invoice_number), 
        cost = ISNULL(?, cost), 
        sale_fee = ISNULL(?, sale_fee), 
        updated = ISNULL(?, updated)
    WHERE order_id = ?
    """
